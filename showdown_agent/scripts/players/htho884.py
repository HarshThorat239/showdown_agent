...
class CustomAgent(Player):
    ...
    def _find_move(self, battle, *names):
        targets = {self._norm(n) for n in names}
        for mv in (battle.available_moves or []):
            if self._norm(self._move_name(mv)) in targets:
                return mv
        return None

    def teampreview(self, battle: AbstractBattle) -> str:
        # Reset battle-specific state at the start of each battle (for testing)
        self.toxic_spikes = 0
        self.last_opponent_species = None
        self.opponent_just_switched_after_ko = False
        self.last_opponent_hp = None
    ...
    def type_multiplier(self, atk_type, def_types, battle=None):
        ...
        if battle and hasattr(battle, 'weather') and battle.weather:
            weather_name = str(battle.weather.get('name', '') if isinstance(battle.weather, dict) else battle.weather).lower()
            if weather_name == 'sun' and atk == PokemonType.FIRE:
                mult *= 1.5
            elif weather_name == 'rain' and atk == PokemonType.WATER:
                mult *= 1.5
    ...
    def would_be_immune_to_candidate_moves(self, candidate_pokemon, opp_types, opp_ability):
        # """
        # Check if the opponent would be immune to the candidate's move types.
        # This gives the candidate a defensive advantage.
        # """
        if not candidate_pokemon.types or not opp_types:
            return False, 0.0
        ...
        # Check if opponent is immune to any of candidate's move types
        for candidate_type in candidate_pokemon.types:
            if candidate_type is None:
                continue
                
            # Check if opponent has ability-based immunity to this type
            if opp_ability and self.is_move_immune(candidate_type, opp_ability):
                immunity_score += 1.5
                print(f"DEBUG: {candidate_pokemon.species} gets immunity bonus - opponent immune to {candidate_type} due to {opp_ability}")
                continue
                
            # Check type chart immunities (0x effectiveness)
            if hasattr(self, 'type_multiplier'):
                mult = self.type_multiplier(candidate_type, opp_types, None)
                if mult == 0.0:
                    immunity_score += 1.0
                    print(f"DEBUG: {candidate_pokemon.species} gets immunity bonus - opponent type-immune to {candidate_type}")
        
        return immunity_score > 0.0, immunity_score

    def get_resistance_bonus(self, candidate_pokemon, opp_types, battle):
        # """
        # Check if the candidate resists the opponent's move types.
        # This gives the candidate a defensive advantage.
        # Includes weather-based damage reduction from abilities.
        # """
        if not candidate_pokemon.types or not opp_types:
            return 0.0
        ...
        # Check if candidate resists opponent's move types
        for opp_type in opp_types:
            if opp_type is None:
                continue
                
            # Check type chart resistances (0.5x effectiveness)
            if hasattr(self, 'type_multiplier'):
                mult = self.type_multiplier(opp_type, candidate_pokemon.types, battle)
                
                # Check for weather-based damage reduction from abilities
                weather_reduction = 1.0
                candidate_ability = getattr(candidate_pokemon, 'ability', None)
                if candidate_ability:
                    ability_name = str(candidate_ability).lower()
                    
                    # Orichalcum Pulse creates harsh sunlight, reducing Water damage by 50%
                    if ability_name == 'orichalcumpulse' and opp_type == PokemonType.WATER:
                        weather_reduction = 0.5
                        print(f"DEBUG: {candidate_pokemon.species} Orichalcum Pulse reduces Water damage by 50%")
                    
                    # Drizzle creates rain, reducing Fire damage by 50%
                    elif ability_name == 'drizzle' and opp_type == PokemonType.FIRE:
                        weather_reduction = 0.5
                        print(f"DEBUG: {candidate_pokemon.species} Drizzle reduces Fire damage by 50%")
                
                # Calculate final effectiveness including weather
                final_mult = mult * weather_reduction
                
                if final_mult == 0.25:
                    resistance_score += 1.5  # Double resistance + weather = very strong
                    print(f"DEBUG: {candidate_pokemon.species} gets strong resistance bonus - {opp_type} moves are {final_mult}x effective (type: {mult}x, weather: {weather_reduction}x)")
                elif final_mult == 0.5:
                    resistance_score += 0.5
                    print(f"DEBUG: {candidate_pokemon.species} gets resistance bonus - resists {opp_type} moves ({final_mult}x effective)")
                elif mult == 0.5 and weather_reduction < 1.0:
                    resistance_score += 0.75  # Type resistance + weather bonus
                    print(f"DEBUG: {candidate_pokemon.species} gets enhanced resistance bonus - {opp_type} moves are {final_mult}x effective (type: {mult}x, weather: {weather_reduction}x)")
    ...
    def pick_best_switch(self, battle, opp_types, current_pokemon=None, super_effective_moves=None):
        ...
        if is_forced_switch:
            ...
            # Calculate immunity bonus for current Pokemon (same as candidates)
            current_immunity_bonus = 0.0
            
            # Check if current Pokemon is immune to opponent's likely moves
            has_immunity, immunity_score = self.would_be_immune_to_opponent_moves(current_pokemon, opp_types, opp_ability)
            if has_immunity:
                current_immunity_bonus += immunity_score
                print(f"DEBUG: Current {current_pokemon.species} gets enhanced immunity bonus: {immunity_score:.2f}")
            
            # Check if opponent is immune to current Pokemon's move types (reverse immunity)
            has_reverse_immunity, reverse_immunity_score = self.would_be_immune_to_candidate_moves(current_pokemon, opp_types, opp_ability)
            if has_reverse_immunity:
                current_immunity_bonus += reverse_immunity_score
                print(f"DEBUG: Current {current_pokemon.species} gets reverse immunity bonus: {reverse_immunity_score:.2f}")
            
            # Check if current Pokemon resists opponent's move types
            resistance_bonus = self.get_resistance_bonus(current_pokemon, opp_types, battle)
            if resistance_bonus > 0.0:
                current_immunity_bonus += resistance_bonus
                print(f"DEBUG: Current {current_pokemon.species} gets resistance bonus: {resistance_bonus:.2f}")

            # Check for weather-based immunities for current Pokemon
            weather_immunity_bonus = self.check_weather_immunities(current_pokemon, battle)
            if weather_immunity_bonus > 0.0:
                current_immunity_bonus += weather_immunity_bonus
                print(f"DEBUG: Current {current_pokemon.species} gets weather immunity bonus: {weather_immunity_bonus:.2f}")
            
            # Check for status immunities for current Pokemon
            statusy = any(t in [PokemonType.POISON, PokemonType.GHOST, PokemonType.GRASS, PokemonType.ELECTRIC]for t in opp_types if t is not None)
            if statusy:
                status_immunity_bonus = self.check_status_immunities(current_pokemon, opp_types)
                if status_immunity_bonus > 0.0:
                    current_immunity_bonus += status_immunity_bonus
                    print(f"DEBUG: Current {current_pokemon.species} gets status immunity bonus: {status_immunity_bonus:.2f}")
            
            # Check for hazard immunities for current Pokemon
            hazard_immunity_bonus = self.check_hazard_immunities(current_pokemon, battle)
            if hazard_immunity_bonus > 0.0:
                current_immunity_bonus += hazard_immunity_bonus
                print(f"DEBUG: Current {current_pokemon.species} gets hazard immunity bonus: {hazard_immunity_bonus:.2f}")
            
            current_score = current_base_score + current_defensive_bonus + current_bonus + current_immunity_bonus - switch_penalty
            
            print(f"DEBUG: Current {current_pokemon.species} score breakdown: base={current_base_score:.2f}, def_bonus={current_defensive_bonus:.2f}, current_bonus={current_bonus:.2f}, immunity_bonus={current_immunity_bonus:.2f}, switch_penalty={switch_penalty:.2f}, total={current_score:.2f}")
        ...
        for candidate in battle.available_switches:
            ...
            # Check if opponent is immune to candidate's move types (reverse immunity)
            has_reverse_immunity, reverse_immunity_score = self.would_be_immune_to_candidate_moves(candidate, opp_types, opp_ability)
            if has_reverse_immunity:
                immunity_bonus += reverse_immunity_score
                print(f"DEBUG: {candidate.species} gets reverse immunity bonus: {reverse_immunity_score:.2f}")
            
            # Check if candidate resists opponent's move types
            resistance_bonus = self.get_resistance_bonus(candidate, opp_types, battle)
            if resistance_bonus > 0.0:
                immunity_bonus += resistance_bonus
                print(f"DEBUG: {candidate.species} gets resistance bonus: {resistance_bonus:.2f}")
    ...
    def choose_move(self, battle: AbstractBattle):
        ...
        for mv in battle.available_moves:
            move_type = getattr(mv, 'type', None) or getattr(mv, 'type_id', None)
            if move_type is None:
                continue

            move_name = self._move_name(mv)
            bp = getattr(mv, 'base_power', 0) or 0
        ...
        if me.species == 'eternatus' and me.current_hp_fraction  < 0.45:
            for mv in battle.available_moves:
        ...
        if me.item == 'choicescarf':
            ...
            if not super_effective_moves and not neutral_moves and resisted_moves:
                ...
                best_switch = self.pick_best_switch(
                    battle,
                    current_pokemon=None,  # Ignore the current Choice Scarf Pokemon in switch evaluation
        ...
        if self.opponent_just_switched_after_ko:
            ...
            if high_effective_moves:
                ...
                move_name = self._move_name(best_high_move[0])
                print(f"DEBUG: *** USING {best_high_move[2]}x EFFECTIVE MOVE: {move_name} vs {opp.species} at {opp_hp:.1%} HP ***")

                # --- Surf override (robust naming) ---
                if (str(me.species or '').lower() == 'kyogre' and 
                    (me.current_hp_fraction or 1.0) < 0.6 and 
                    self._norm(move_name) in ('waterspout', 'waterspout')):  # handles 'Water Spout'
                    surf_mv = self._find_move(battle, 'surf')
                    if surf_mv:
                        print(f"DEBUG: Kyogre HP {me.current_hp_fraction:.1%} < 60% - overriding Water Spout with Surf")
                        return self.create_order(surf_mv)
                    print(f"DEBUG: Surf not found, proceeding with {move_name}")
                return self.create_order(best_high_move[0])
            
            # Check if opponent is below 80% HP
            # Use x2/x4 effective moves if opponent HP < 80% OR it's their last Pokemon
            is_last_opp_pokemon = hasattr(battle, "opponent_team") and len([p for p in getattr(battle, "opponent_team", []) if not getattr(p, "fainted", False)]) <= 1
            if opp_hp < 0.8 or is_last_opp_pokemon:
                print(f"DEBUG: *** Opponent {opp.species} is at {opp_hp:.1%} HP or is last Pokemon - checking for x4/x2 effective moves ***")
                ...
                if x2_effective_moves:
                    ...
                    move_name = self._move_name(best_x2_move[0])
                    print(f"DEBUG: *** USING x2 EFFECTIVE MOVE: {move_name} vs {opp.species} at {opp_hp:.1%} HP ***")

                    # --- Surf override (robust naming) ---
                    if (str(me.species or '').lower() == 'kyogre' and 
                        (me.current_hp_fraction or 1.0) < 0.6 and 
                        self._norm(move_name) in ('waterspout', 'waterspout')):
                        surf_mv = self._find_move(battle, 'surf')
                        if surf_mv:
                            print(f"DEBUG: Kyogre HP {me.current_hp_fraction:.1%} < 60% - overriding Water Spout with Surf")
                            return self.create_order(surf_mv)
        ...
        if hasattr(opp, 'base_stats') and opp.base_stats.get('hp', 100) < 35:
            ...
            if neutral_moves_available:
                ...
                move_name = self._move_name(best_neutral_move[0])
        ...
        if super_effective_moves:
            ...
            move_obj = best_se_move[0]
            move_name = self._move_name(move_obj)
        ...
        if resisted_moves:
            ...
            move_name = self._move_name(best_resisted_move[0])
